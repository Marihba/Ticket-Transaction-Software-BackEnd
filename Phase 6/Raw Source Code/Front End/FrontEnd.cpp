...
int main(int argc, char* argv[]) {
	...
	string output_file = argv[3]; // "transaction_stream<#>.output"
	...
	do {
		...
		if (command == kExitKeyword) {
			...
			if (command == "logout") {
				...
				BasicTransaction logout_transaction(current_user, current_user_accounts_file, output_file);
				current_user = logout_transaction.Execute(logout_transaction.kEndSessionTransactionCode);
				cout << "Logout successful.\n";
				is_logged_in = false;
			} else if (command == "addcredit") {
				BasicTransaction addcredit_transaction(current_user, current_user_accounts_file, output_file);
				...
				// Create a refund transaction
				 RefundTransaction refund_transaction(current_user, current_user_accounts_file, output_file);
				...
				TicketTransaction buy_transaction(current_user, tickets_file, output_file);
				...
				TicketTransaction sell_transaction(current_user, tickets_file, output_file);
				...
				BasicTransaction create_transaction(current_user, current_user_accounts_file, output_file);
				...
				BasicTransaction delete_transaction(current_user, current_user_accounts_file, output_file);
