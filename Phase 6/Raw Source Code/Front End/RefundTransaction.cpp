  ...
  if ((credit > 0) && ValidUsername(buyer_username) && ValidUsername(seller_username)) {
		...
		if (seller_credit >= credit) {
			...
			if (ParseAccountData(kKeyAccount, buyer_info) != SELL_STANDARD_CODE) {
				SaveToOutput(kRefundTransactionCode, output_file_, NULL);
				return;
			}
		}
	}
...
RefundTransaction::RefundTransaction(Account current_user, string current_user_accounts_file, string output_file) {
