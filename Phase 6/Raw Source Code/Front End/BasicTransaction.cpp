...
BasicTransaction::BasicTransaction(Account account, string current_user_accounts_file, string output_file) {
	account_ = account;
...
void BasicTransaction::Create(string new_account_name, string new_account_type, double new_account_credit){
	...
	if ((!deleted) && (valid_credit) && (valid_name) && (!found) && (valid_type)){
		...
		SaveToOutput(kCreateTransactionCode, output_file_, new_user);
	}
}
...
void BasicTransaction::Delete(Account* account){
	...
	if (!exists && found && (account->GetName() != account_.GetName())){
			...
			SaveToOutput(kDeleteTransactionCode, output_file_, account);
	}
}
...
void BasicTransaction::AddCreditsAndSave(Account* account, double amount) {
	...
	if ((amount > 0) && (amount <= kMaxCreditToAdd)) {
		...
		//SaveToOutput(kAddCreditTransactionCode, output_file_, account);
		double temp = account->GetCredit();
		account->SetCredit(amount);
		// Only save the new number to output file
		SaveToOutput(kAddCreditTransactionCode, output_file_, account);
		account->SetCredit(temp + amount);
...
Account BasicTransaction::Execute(int transaction_code) {
	...
	if (transaction_code == kEndSessionTransactionCode) {
		SaveToOutput(transaction_code, output_file_, NULL);
		...
		if (valid_type && valid_credit && IsValidCreate(new_account_name, new_account_type, new_account_credit)) {
			//cout << "." << new_account_name << ".\n";
			//cout << "." << new_account_type << ".\n";
			//cout << "." << new_account_credit << ".\n";
